...
The `webmeetLivekitEgress` agent owns recording and egress worker execution. It may receive generated egress config and write recordings only to the declared recording volume. The shared recording host path is `.ploinky/data/webmeet/recordings`, mounted at `/data/recordings` so WebMeet-controlled recording paths stay under a semantic data directory inside the container.
...
The generated egress config is written on the host under `.ploinky/agents/webmeetLivekitEgress/egress.yaml` and mounted in the container at `/working-data/generated/egress.yaml`. The `EGRESS_CONFIG_FILE` default must point at that generated path.
