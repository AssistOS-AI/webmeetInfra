...
`WEBMEET_LIVEKIT_REDIS_ADDRESS` is the LiveKit Redis address written into `redis.address` of the generated `livekit.yaml`. The profile defaults reflect how LiveKit reaches Redis on each topology: the `default` and `dev` profiles default to `webmeetRedis:6379` because LiveKit and Redis share the `webmeet` bridge network, while the `prod` profile explicitly defaults to `127.0.0.1:6379` because host-network LiveKit consumes Redis through Redis's published host port. Operators may override the value per workspace if the Redis topology changes.
